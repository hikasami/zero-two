...
class admins(commands.Cog):
    ...
    async def kick_user(self, ctx: disnake.ApplicationCommandInteraction, user: disnake.Member, reason: str = None):
        ...
        if user.top_role >= ctx.author.top_role:
            await ctx.send("Вы не можете выгнать пользователя с более высокой или равной ролью.")
            return

        await user.kick(reason=reason)
        try:
            await user.send(f"<:icons_kick:1223137566123884606> Вы были выгнаны с сервера {ctx.guild.name}. Причина: {reason}")
    ...
    async def clear(self, ctx, amount: int):
        ...
        if amount > 1000:
            await ctx.send('Нельзя удалить больше 1000 сообщений за раз.')
            return

        deleted = await ctx.channel.purge(limit=amount)
        embed = disnake.Embed(color=0x2f3136)
        embed.add_field(name="Очистка чата", value=f"Было удалено {len(deleted)} сообщений", inline=False)
        await ctx.send(embed=embed, ephemeral=True)
    ...
    async def ban_user(self, ctx, user: disnake.Member, reason: str = None):
        ...
        if banned_user:
            embed = disnake.Embed(title="Бан", description=f"<:icons_ban:1223137191237255218> {user.mention} Этот пользователь уже забанен.", color=0x2f3136)
        else:
            await user.ban(reason=reason)
            c.execute("INSERT INTO bans (user_id, username, reason) VALUES (?, ?, ?)", (user.id, user.name, reason))
            conn.commit()
            embed = disnake.Embed(title="Бан", description=f"<:icons_ban:1223137191237255218> Пользователь {user.mention} успешно заблокирован.", color=0x2f3136)
            try:
    ...
    @commands.slash_command(name="message_bot", description="Отправить сообщение от имени бота.")
    ...
    @commands.slash_command(name='stay', description='Оставаться боту в голосовом канале')
    @commands.has_permissions(administrator=True)
    async def stay(ctx):
        if not ctx.author.voice:
            embed = disnake.Embed(
    ...
    async def poll(self, ctx, *, text):
        ...
        await msg.add_reaction("<:icons_Correct:1223137153140392018>")
        await msg.add_reaction("<:icons_Wrong:1223137173797343243>")
